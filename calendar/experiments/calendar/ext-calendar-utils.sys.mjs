...
export function convertItem(item, options, extension) {
  ...
  // Extract common properties for both events and tasks
  try {
    props.item = {};

    // Summary/title
    if (item.getProperty("Summary") !== undefined) {
      props.item.summary = item.getProperty("Summary");
    }

    // Description
    if (item.getProperty("Description") !== undefined) {
      props.item.description = item.getProperty("Description");
    }

    // Start date
    if (item.startDate) {
      props.item.startDate = item.startDate.icalString;
    }

    // Due date (for tasks) or end date (for events)
    if (item.getProperty("DUE")) {
      props.item.dueDate = item.getProperty("DUE").toString();
    } else if (item.endDate) {
      props.item.endDate = item.endDate.icalString;
    }

    // Categories
    if (item.getCategories() !== undefined) {
      props.item.categories = Array.from(item.getCategories());
    }
    ...
    // Created date
    if (item.createdDate) {
      props.item.createdDate = item.createdDate.icalString;
    }

    // Last modified date
    if (item.lastModifiedDate) {
      props.item.lastModifiedDate = item.lastModifiedDate.icalString;
    }

    // URL
    if (item.url) {
      props.item.url = item.url.spec;
    }

    // Location
    if (item.location) {
      props.item.location = item.location;
    }

    // Attendees (for events)
    if (item.attendees && item.attendees.length > 0) {
      props.item.attendees = Array.from(item.attendees).map((attendee) => ({
    ...
    if (item.recurrenceInfo) {
      props.item.recurrenceInfo = {
        isRecurring: !!item.recurrenceInfo,
      };
    }
  } catch (ex) {
    // If we fail to extract properties, still return basic info
    console.warn("Failed to extract some calendar item properties:", ex);
    // props.item = {};
  }
