      ...
      calendar: {
        items: {
          ...
          async update(calendarId, id, updateProperties) {
            ...
            //TODO: RATIQ possible fix here for new task not getting their status assigned correctly
            let updatedIcs;
            if (oldItem.icalString.includes("STATUS")) {
              updatedIcs = oldItem
                .clone()
                .icalString.replace(
                  /^STATUS:.*$/m,
                  "STATUS:" + updateProperties.item.status,
                );
            } else {
              updatedIcs = oldItem
                .clone()
            ...
            updateProperties.item = updatedIcs;
            updateProperties.format = "ical";
            ...
            // Apply update properties if they exist
            if (updateProperties.item) {
              const updateItem = updateProperties.item;

              // Update categories - this is what we're primarily interested in for Kanban
              if (
                updateItem.categories !== undefined &&
                Array.isArray(updateItem.categories)
              ) {
                newItem.categories.clear();
                for (const category of updateItem.categories) {
                  newItem.categories.add(category);
                }
              }

              // TODO: Handle other property updates if needed in the future
            }
          ...
          async openEditor(calendarId, id) {
            const calendar = getResolvedCalendarById(
              context.extension,
              calendarId,
            );
            ...
            if (!win) {
              throw new ExtensionError("Could not find a Thunderbird window");
            }

            win.modifyEventWithDialog(item);
          },
